...
namespace Dto.Writer.Logic
{
    public class CodeGenerator : ICodeGenerator
    {
        public string GenerateSourcecode(FileInfo fileInfo, string outFilePath)
        {
            ...
            foreach (var model in fileInfo.Classes.Where(c => c.IsEnabled))
            {
                ...
                // 只有一个类的话，使用与文件名一致的类名
                if (fileInfo.Classes.Count == 1)
                {
                    className = System.IO.Path.GetFileNameWithoutExtension(outFilePath);
                }

                var dtoName = $"{className}{Constants.DtoSuffix}";
